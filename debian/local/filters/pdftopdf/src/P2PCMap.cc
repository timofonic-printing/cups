...
P2PCMap *P2PCMap::parse(P2PCMapCache *cache, GooString *collectionA,
		  GooString *cMapNameA) {
  FILE *f;
  P2PCMap *cmap;
  PSTokenizer *pst;
  ...
  if (!(f = globalParams->findCMapFile(collectionA, cMapNameA))) {
    ...
    if (!cMapNameA->cmp("Identity") || !cMapNameA->cmp("Identity-H")) {
      return new P2PCMap(collectionA->copy(), cMapNameA->copy(), 0);
    }
  ...
  cmap = new P2PCMap(collectionA->copy(), cMapNameA->copy());
...
P2PCMap::P2PCMap(GooString *collectionA, GooString *cMapNameA) {
  int i;
...
P2PCMap::P2PCMap(GooString *collectionA, GooString *cMapNameA, int wModeA) {
  collection = collectionA;
  cMapName = cMapNameA;
  wMode = wModeA;
  vector = NULL;
...
void P2PCMap::useCMap(P2PCMapCache *cache, char *useName) {
...
void P2PCMap::copyVector(CMapVectorEntry *dest, CMapVectorEntry *src) {
  int i, j;
...
void P2PCMap::addCodeSpace(CMapVectorEntry *vec, Guint start, Guint end,
			Guint nBytes) {
  Guint start2, end2;
...
void P2PCMap::addCIDs(Guint start, Guint end, Guint nBytes, CID firstCID) {
  CMapVectorEntry *vec;
  CID cid;
  int byte;
  Guint i;
...
void P2PCMap::freeCMapVector(CMapVectorEntry *vec) {
  int i;
...
GBool P2PCMap::match(GooString *collectionA, GooString *cMapNameA) {
  return !collection->cmp(collectionA) && !cMapName->cmp(cMapNameA);
}

CID P2PCMap::getCID(char *s, int len, int *nUsed) {
  CMapVectorEntry *vec;
  int n, i;
...
void P2PCMap::setReverseMapVector(Guint startCode, CMapVectorEntry *vec,
...
void P2PCMap::setReverseMap(Guint *rmap, Guint rmapSize, Guint ncand) {
  setReverseMapVector(0,vector,rmap,rmapSize,ncand);
}

//------------------------------------------------------------------------

P2PCMapCache::P2PCMapCache() {
  int i;
...
P2PCMap *P2PCMapCache::getCMap(GooString *collection, GooString *cMapName) {
  P2PCMap *cmap;
  int i, j;
  ...
  if ((cmap = P2PCMap::parse(this, collection, cMapName))) {
