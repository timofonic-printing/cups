...
void SplashClip::getBBox(int *xMinA, int *yMinA, int *xMaxA, int *yMaxA) {
  int i;
  int cxMin = xMin, cyMin = yMin, cxMax = xMax, cyMax = yMax;
...
SplashPath *SplashClip::makePath() {
  int i,j;
  int y, x0, x1;
  int txMin, tyMin, txMax, tyMax;
  Guchar *cbuf,*tbuf;
  ...
  scanners[0]->getNextSpan(tyMin-1,&x0,&x1);
