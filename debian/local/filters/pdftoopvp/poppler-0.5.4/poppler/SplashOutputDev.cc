...
void SplashOutputDev::patternStroke(GfxState *state, SplashPath *orgPath) {
  int width, height;
  ...
  for (h = 0;h < height;h += SLICE_FOR_PATTERN) {
    ...
    tsplash = new Splash(tbitmap);
    splash = tsplash;
    color[0] = 0;
    tsplash->setStrokePattern(new SplashSolidColor(color));
    color[0] = 0xff;
    tsplash->clear(color);
...
void SplashOutputDev::patternFillChar(GfxState *state,
...
GfxPath *SplashOutputDev::bitmapToGfxPath(SplashBitmap *bitmapA,
    int width, int height)
{
  int x,y;
  GfxPath *path;
...
GfxPath *SplashOutputDev::splashPathToGfxPath(SplashPath *path)
{
  GfxPath *gfxPath;
  int i;
...
void SplashOutputDev::patternFillImageMask(GfxState *state,
