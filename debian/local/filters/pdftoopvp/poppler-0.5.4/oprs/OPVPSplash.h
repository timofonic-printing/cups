...
class OPVPClipPath {
public:
  OPVPClipPath(SplashPath *pathA, GBool eoA);
  void push();
  static OPVPClipPath *pop();
  ~OPVPClipPath() { delete path; }
  SplashPath *getPath() { return path; }
...
private:
  SplashError makePath(SplashPath *path);
  void makeBrush(SplashPattern *pattern, opvp_brush_t *brush);
  SplashError doClipPath(SplashPath *path, GBool eo,
     OPVPClipPath *prevClip);
  opvp_cspace_t getOPVPColorSpace();
  GBool equalPattern(SplashPattern *pt1, SplashPattern *pt2);
  GBool isRectanglePath(SplashPath *path, SplashCoord *xMin,
    SplashCoord *yMin, SplashCoord *xMax, SplashCoord *yMax);
  SplashError makeRectanglePath(SplashCoord x0, SplashCoord y0,
    SplashCoord x1, SplashCoord y1, SplashPath **p);
  ...
  SplashError strokeByMyself(SplashPath *path);
  SplashError fillByMyself(SplashPath *path, GBool eo);
