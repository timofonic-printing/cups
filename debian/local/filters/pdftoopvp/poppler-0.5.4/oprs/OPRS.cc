...
SplashClip *OPRS::getClip() {
  return SPLASH(getClip());
}
...
SplashError OPRS::clipToPath(SplashPath *path, GBool eo) {
...
SplashError OPRS::stroke(SplashPath *path) {
    return SPLASH(stroke(path));
}

SplashError OPRS::fill(SplashPath *path, GBool eo) {
...
SplashError OPRS::fillGlyph(SplashCoord x, SplashCoord y,
    ...
    return SPLASH(fillGlyph(x,y,glyph));
}

SplashError OPRS::fillImageMask(SplashImageMaskSource src, void *srcData,
				  int w, int h, SplashCoord *mat) {
    return SPLASH(fillImageMask(src,srcData,w,h,mat));
}
...
SplashError OPRS::drawImage(SplashImageSource src, void *srcData,
			      ...
			      int w, int h, SplashCoord *mat) {
    if (rasterMode) {
	return splash->drawImage(src,srcData,srcMode,w,h,mat);
    } else {
	return opvpSplash->drawImage(src,srcData,srcMode,w,h,mat);
    }
}
...
int OPRS::init(const char *driverName, int outputFD,
  const char *printerModel,int rasterModeA, int nOptions,
  char *optionKeys[], char *optionVals[])
{
    ...
    if (opvp == 0) return -1;
    if (!opvp->supportNewPath) {
	/* force set rater mode */
	rasterModeA = gTrue;
    }
    rasterMode = rasterModeA;
    if (!rasterMode) {
...
int OPRS::setColorMode(int colorModeA)
{
